...
class OrderHandler : public HTTPRequestHandler {
 public:
  ...
  bool checkServiceExists(HTTPServerRequest& request, long service_id) {
    HTMLForm form(request, request.stream());
    std::string scheme;
    std::string info;
    std::string host = "localhost";
    ...
    url = "http://" + host + ":8081/get?id=" + std::to_string(service_id);
  ...
  void handleGet(HTTPServerRequest& request, HTTPServerResponse& response, long user_id) {
    ...
    database::Order order = database::Order::read_by_user_id(user_id);
  ...
  void handleAdd(HTTPServerRequest& request, HTTPServerResponse& response, long user_id) {
    ...
    if (form.has("id")) {
      long service_id = atol(form.get("id").c_str());
      if (!checkServiceExists(request, service_id)) {
        response.setStatus(Poco::Net::HTTPResponse::HTTPStatus::HTTP_BAD_REQUEST);
        response.setChunkedTransferEncoding(true);
        response.setContentType("application/json");
        Poco::JSON::Object::Ptr root = new Poco::JSON::Object();
        root->set("type", "/errors/bad_request");
        root->set("title", "Service with such ID is not found");
        root->set("detail", "Service with such ID is not found");
      ...
      order.service_ids().push_back(service_id);
      order.save_to_mysql();
      ...
      response.setContentType("application/json");
      Poco::JSON::Object::Ptr root = new Poco::JSON::Object();
      root->set("service_id", std::to_string(service_id));
